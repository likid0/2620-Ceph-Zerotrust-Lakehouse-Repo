...
. Apply fine‑grained access control by issuing time‑bound credentials through Polaris; table‑level policies are enforced directly by *Ceph Object Gateway (RGW)*, Ceph’s native support for S3 IAM and STS APIs enables us to issue scoped, short-lived tokens without custom proxies or third-party plugin
...
Underlying all these steps is IBM Storage Ceph—a scale-out object storage platform whose rich S3 API, native IAM/ST​S implementation, and high-throughput architecture make it the perfect foundation for modern lakehouses.
...
While object storage systems vary widely in their feature set, Ceph stands out
because its S3-compatible Ceph Object Gateway (RGW) offers **native IAM constructs**: accounts, users, roles, and STS tokens—making it possible to integrate identity-aware workflows without external components.
...
|*Enforce @ Ceph* |*Ceph Object Gateway (RGW)* evaluates that policy – no extra proxy hop required.
|===

This is possible because Cephi Object Gateway (RGW) implements AWS-style IAM semantics—down to ARNs, trust policies, and token vending—so existing AWS tooling and Terraform providers work transparently against Ceph.
 ...
 ** Credentials are short-lived, valid for minutes, not months
 ** And with the least privileges required
* **Engine-agnostic** – Spark, Trino, Flink, etc, all speak the same Iceberg metadata, so one policy fits all, The Iceberg catalog is the source of truth.
* **Full speed** – the executor streams Parquet directly from *Ceph Object Gateway (RGW)*, no proxys needed.
...
CSV Batch to *Ceph Object Gateway (RGW)* containing the previous day *product
sales* data, the shop dataset contains the following columns:

[cols="1,3", options="header"]
|===
| Column Name     | Description
| `product_id`    | Unique identifier for the sold product
| `category`      | Product department (e.g., dairy, produce, pantry)
| `price`         | Unit price of the product (in local currency)
| `quantity`      | Number of units sold
| `email`         | Customer email address from loyalty program (PII)
...
The glue that enforces who can access what is the **Polaris** Iceberg RESTful Catalog.
...
. *From the Terminal CLI* – create *Ceph Object Gateway (RGW)* account & root user.
...
| *Storage*
| Create Ceph *Ceph Object Gateway (RGW)* buckets and IAM roles from the dashboard and Terraform.
...
* Pre‑deployed IBM Storage Ceph cluster with *Ceph Object Gateway (RGW)* running, that provides our required S3 Endpoint.  
...
From the previous command we can verify that we have a healthy running Ceph Cluster amd it has *Ceph Object Gateway (RGW)* , S3 endpoint,  deployed and active
...
the Ceph *Ceph Object Gateway (RGW)* must know **which tenant it belongs to** and **who the root
...
The command below runs on our Workstation CLI terminal:
...
* Creatre a new account named `analytics` in the *Ceph Object Gateway (RGW)* metadata store
...
== Configure and Run the Terraform Automation Code to Create Required Ceph Object Gateway (RGW) resources
...
| Collect user-specific inputs such as the Ceph S3/STS endpoint, the credentials profile that can talk to *Ceph Object Gateway (RGW)*, and the bucket name that will back the Polaris catalog.

| *AWS provider configured for Ceph*
| Uses the standard `hashicorp/aws` provider but points its `s3`, `sts`, and `iam` endpoints to your S3 Object Ceph cluster.
...
- `account_arn` – Ceph RGW account ID (used in later trust policies)
- `location` – `s3://…` URI Polaris will register as its warehouse
- `role_arn` – ARN of the client role
- `admin_access_key` / `admin_secret_key` – keys for the admin user
  (the secret is marked *sensitive*, so Terraform hides it in plan logs)
|===

These constructs are implemented natively by Ceph Object Gateway (RGW) meaning governance and least-privilege enforcement live directly in the storage layer, not in an overlay or external proxy.
...
You only need to modify the RGW `Account ID` to match the `Account ID` you created
...
====
**Data stays in Ceph.**
All compute containers are ephemeral; the authoritative data lives durably in your IBM Storage Ceph cluster (erasure-coded, replicated, and accessible via S3/STS). You can tear down and rebuild the analytics stack without re-seeding data.
====
...
(Spark, Trino, Superset) Containers running.  What it still lacks is a
...
*Nightly batch of shops product‑movement in CSV format sent to the IBM Storage
Ceph Object Central Cluster → All PII is masked → in the morning data dashboard visualizations are available for the stakeholders to take action.*
...
| *Engineer*   | Read / Write access to **RAW**
| *Compliance* | Read access to **RAW**, Read / Write access to **GOLD**
| *Analyst*    | Read-only access to **GOLD**
| *Admin*      | Full privileges (`catalog_admin`)
|===

* Authentication to the catalog happens with **Short‑lived OAuth 2 tokens** for each persona, exported as Terraform
...
In the left sidebar, navigate to the `notebooks/` directory and double-click on `polaris_data_pipeline.ipynb` to open it.
...
You can run cells one by one clicking the > button on the dashboar(or the
keyboard shortcut of: SHIFT+Enter) , and waiting to get the output before running the next cell:

IMPORTANT: Step 2 of the notebook `Spark Sessions setup` may take around 1 minute to produce output

example output you should see once the Spark Sessions setup has finished: 

image::spark_screen1.png[spark,600]

Then you can run one by one each cell waiting for the outpus to show.
...
Thanks to Polaris’s REST‑based Iceberg catalog, you can point **any** Iceberg Compatible SQL engine at the same tables and get the **exact** same schema, data, and fine‑grained access controls. Because the underlying objects sit in IBM Storage Ceph, multiple engines can read (and, where permitted, mutate) the same Iceberg tables in parallel without copying data between silos.
...
Trino will report how many rows were deleted. Somithing like the following
output, meaning it has deleted 2 rows that had duplicate entries.,

[source,plain]
----
DELETE: 2 rows. 
Query 20250715_185908_00003_gbm67, FINISHED, 1 node 
...
This illustrates the power of a unified, governed Iceberg catalog for
multi‑engine analytics and also IBM Storage Ceph’s rich S3 feature set—complete with IAM, STS, and fine-grained bucket policies—proved essential for building a zero-trust lakehouse without proprietary gateways or add-ons. Its ability to combine cloud-native governance APIs with enterprise-grade scalability makes it the ideal backbone for secure, multi-engine analytics.
...
Log in to Superset from your lab workstation Firefox browser:
...
Click **Save ▶** and choose **Save dataset**.
...
You may find a pop-up asking to refres/update the dashboard after your updates
like this one 

image::update_chart.png[refresh,800]
...
[sidebar.ceph-spotlight]
****
**IBM Storage Ceph — the data‑platform backbone**

Ceph turns every step of this lab from _possible_ to _trivial_:

* **Scale‑out buckets** – add nodes and performance + capacity grows linearly.
* **Durability first** – erasure coding and replica sets protect every object
  Localy, with the option to do multisite async replication to match DR
  requirements.
* **Full S3 / IAM / STS fidelity** – the same Terraform modules, AWS CLI calls, and short‑lived tokens you use in the public‑cloud work here unchanged.
* **Policy enforcement in‑flight** – RGW evaluates Polaris‑issued credentials at wire speed; no proxy needed.
* **Unified object, block, and file** – one cluster you can extend beyond this workshop.

That solid foundation lets us focus on governance and analytics instead of storage plumbing.
****
