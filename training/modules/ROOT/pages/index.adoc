...
== Lab Introduction
Are you ready to see how **Zero‑Trust security** and **open‑format analytics**
come together with IBM Storage Ceph as the common data platform for the solution?

image::ceph_Datalake.png[float="right",role=padded,width=190]

In the next ~90 minutes you’ll stand up a miniature lakehouse, secure it
with Polaris, push data through a Spark pipeline, query it with Trino,
and build a Superset dashboard.

*Are you Ready?*

If you want an introduction on the subject to gain context move to chapter 3
"ZeroTrus Data LakeHouse". If you prefer to get your hands dirty straight away
Jump to Chapter 4 and let’s build a Zero‑Trust Data Lakehouse!
...
image::add4.png[float="right",role=padded,width=300]
